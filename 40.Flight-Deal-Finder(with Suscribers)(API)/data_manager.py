...
class DataManager:
    ...
    def register(self):
        ...
        sheet_url_to_post = "https://api.sheety.co/52e4797d4ba4a030a600379c091918dd/flightDeals/users"
        search_value = self.new_fly[2]  # the email
        headers = {
            "Authorization": f"Bearer {self.token}",
            "Content-Type": "application/json"
        }
        ...
        if response.status_code == 200:
            ...
            for row in data:
                if row["email"].lower() == search_value.lower():
                    print("You are already in the club!")
                    return 0
                # Create new row
            new_data = {
                "firstName": self.new_fly[0],
                "lastName": self.new_fly[1],
                "email": search_value,
            }
            # Make POST request to add row
            response = requests.post(url=sheet_url_to_post, headers=headers, json={"user": new_data})
